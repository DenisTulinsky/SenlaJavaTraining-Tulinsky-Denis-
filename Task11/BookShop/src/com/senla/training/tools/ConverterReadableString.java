...
public class ConverterReadableString implements IConverterReadableString {
	...
	public String convert(IBook book) {
		String str = null;
		try {
			str = new StringBuilder().append("Book \n").append("ID: ").append(book.getId()).append(" Title: ")
					...
					.append(book.getArrivalDate()).append(" Description: ").append(book.getDescription()).toString();
		} catch (Exception e) {
	...
	public String convert(IOrder order) {
		String str = null;
		try {
			str = new StringBuilder().append("Order \n").append("ID: ").append(order.getId()).append(" Book title: ")
					.append(order.getBook().getTitle()).append(" Book author: ").append(order.getBook().getAuthor())
					.append(" Customer: ").append(order.getCustomer()).append(" Price: ").append(order.getPrice())
					.append(" Execution status: ").append(order.getStatus()).append(" Execution date ")
					.append(order.getExecutionDate()).toString();
		} catch (Exception e) {
			log.error(e.getMessage());
		}
		return str;
	...
	public String convert(IPreorder preorder) {
		String str = null;
		try {
			str = new StringBuilder().append("Preorder \n").append("ID: ").append(preorder.getId())
					...
					.append(" Execution status: ").append(preorder.getStatus()).toString();
			return str;
		} catch (Exception e) {
			log.error(e.getMessage());
		}
